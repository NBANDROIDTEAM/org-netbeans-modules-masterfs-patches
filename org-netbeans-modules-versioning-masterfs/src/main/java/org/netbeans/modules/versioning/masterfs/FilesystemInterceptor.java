...
/**
 * Plugs into IDE filesystem and delegates file operations to registered
 * versioning systems.
 *
 * @author Maros Sandor
 */
class FilesystemInterceptor extends ProvidedExtensions implements FileChangeListener, VCSAnnotationListener {
     ...
     * is read-only - e.g. when a file is VCS locked and we still want be able
     * to edit and do a silent unlock in the background. The contract with
     * masterfs is that the proper value is determined no matter if the file is
     * managed by a VCS or not.
     *
     * @param file
     * @return
     */
...
//                assert dummy.equals(file);
    ...
    /**
     * There is a contract that says that when a file is locked, it is expected
     * to be changed. This is what openide/text does when it creates a Document.
     * A versioning system is expected to make the file r/w.
     *
